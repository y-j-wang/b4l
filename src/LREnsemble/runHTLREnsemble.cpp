...
/**
 * Logistic Regression for HOLE and TransE
 * @param argc
 * @param argv
 * @return
 */
int main(int argc, char **argv) {
    ...
    if (vm.count("help")) {
        ...
        cout << desc << endl;
        return 0;
    } else if (parameter.hole_restore_path == "" || parameter.transe_restore_path == "") {
        cout << "Please specify path to HOLE and TransE files" << endl << endl;
        cout << desc << endl;
        return 0;
    ...
    if(parameter.weight_path=="") {
        LRUtil::learn_weights_HT(&data, &parameter, hole_E, hole_P, transe_A, transe_R, weights, min_max_values);
    ...
    transform_hole2rescal(hole_P, rescal_R);
    ...
    hit_rate measure = eval_hit_rate(Method::m_RTLREnsemble, &parameter, &data, &hole_E, &rescal_R,
                                     &transe_A, &transe_R, nullptr, nullptr, nullptr, &weights,
                                     parameter.output_path + "/ht_lr_ensemble", &min_max_values);

    pair<value_type, value_type> map = eval_MAP(m_RTLREnsemble, &parameter, &data, &hole_E, &rescal_R, &transe_A,
