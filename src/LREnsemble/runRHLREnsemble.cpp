...
int main(int argc, char **argv) {
    ...
    if (vm.count("help")) {
        ...
        cout << desc << endl;
        return 0;
    } else if (parameter.rescal_restore_path == "" || parameter.hole_restore_path == "") {
        cout << "Please specify path to RESCAL and HOLE files" << endl << endl;
        cout << desc << endl;
        return 0;
    ...
    if(parameter.weight_path=="") {
        LRUtil::learn_weights_RH(&data, &parameter, rescal_A, rescal_R, hole_E, hole_P, weights, descriptor, min_max_values);
    ...
    hit_rate measure = eval_hit_rate(Method::m_RHLREnsemble, &parameter, &data, &rescal_A, &rescal_R,
                                     nullptr, nullptr, &hole_E, &hole_P, &descriptor, &weights,
                                     parameter.output_path + "/rh_lr_ensemble", &min_max_values);

    pair <value_type, value_type> map = eval_MAP(m_RHLREnsemble, &parameter, &data, &rescal_A, &rescal_R, nullptr, nullptr, &hole_E,
