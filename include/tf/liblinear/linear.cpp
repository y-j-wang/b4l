...
//#ifndef std::min
//template <class T> static inline T std::min(T x,T y) { return (x<y)?x:y; }
//#endif
//#ifndef std::max
//template <class T> static inline T std::max(T x,T y) { return (x>y)?x:y; }
//#endif
...
void Solver_MCSVM_CS::Solve(double *w)
{
	...
	if (iter >= max_iter)
		info("\nWARNING: reaching std::max number of iterations\n");
	...
	double Cp, double Cn, int solver_type)
{
	...
	double eps1 = 0.1;
	double min_eps1 = std::min(0.01*eps, eps1);
	int init_candidates_size = 256;
	int max_candidates_size = 4096;
	int candidates_size = std::min(init_candidates_size, max_candidates_size);
	...
	while (iter < max_iter)
	{
		...
		while (t < active_size)
		{
			int send = std::min(candidates_size, active_size-t);
...
#pragma omp parallel for private(s,i) schedule(static)
			...
			if (workingset_size == 0)
				candidates_size = std::min((int)(candidates_size*1.5), max_candidates_size);
			...
			for (s=0; s<workingset_size; s++)
			{
				...
				double alpha_new = std::min(std::max(alpha[i] - G/QD[i], 0.0), C);
				d = alpha_new - alpha[i];
				if (fabs(d) > 1.0e-15)
				{
					alpha[i] = alpha_new;
		...
		if(num_updates_one_iter == 0)
		{
			...
			eps1 = std::max(0.1*eps1, min_eps1);
...
//
//  min_w \sum |wj| + C \sum std::max(0, 1-yi w^T xi)^2,
//
// Given:
// x, y, Cp, Cn
// eps is the stopping tolerance
	...
	double Cp, double Cn)
{
	...
	while(newton_iter < max_newton_iter)
	{
		...
		if(iter >= max_iter)
			info("WARNING: reaching std::max number of inner iterations\n");
	...
	if(newton_iter >= max_newton_iter)
		info("WARNING: reaching std::max number of iterations\n");
